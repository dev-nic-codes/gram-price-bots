...
LOG_PATH = ROOT / "redo-price-bot.log"
...
DEFAULT_ALERT_POOL_ADDRESS = "EQBCwe_IObXA4Mt3RbcHil2s4-v4YQS3wUDt1-DvZOceeMGO"
DEFAULT_ALERT_POOL_LABEL = "STON.fi REDO/GRAM"
DEFAULT_ALERT_DEX = "stonfi"
DEFAULT_ALERT_THRESHOLD_USD = 1000.0
DEFAULT_ALERT_INTERVAL_SECONDS = 20
DEFAULT_ALERT_FETCH_LIMIT = 25
REDO_MASTER_ADDRESS = "0:59fdc69f3f20ebe4a513b3468dc61d194c3864a4464e3662c903648d1a52e6e0"
REDO_FRIENDLY_MASTER_ADDRESS = "EQBZ_cafPyDr5KUTs0aNxh0ZTDhkpEZONmLJA2SNGlLm4Cko"
TOKEN_DECIMALS = 9
...
DEFAULT_CHANNEL = "@redopricess"
DEFAULT_USER_AGENT = "redo-price-bot/2.0"
...
def load_state(env: EnvConfig) -> dict[str, Any]:
    ...
    state["buy_alert_channels"] = normalize_alert_channels(
        state.get("buy_alert_channels") or state["buy_alert_channel"]
    )
    try:
    ...
    state["sell_alert_channels"] = normalize_alert_channels(
        state.get("sell_alert_channels") or state["sell_alert_channel"]
    )
    try:
...
def fetch_dexscreener_market_snapshot_live(runtime: RuntimeConfig) -> MarketSnapshot:
    ...
    if str(base.get("address") or "").strip() != REDO_FRIENDLY_MASTER_ADDRESS:
...
def fetch_ston_pair_layout(runtime: RuntimeConfig, pool_address: str) -> tuple[int, bool]:
    ...
    if asset0 == REDO_FRIENDLY_MASTER_ADDRESS:
        layout = (0, asset1 == STON_NATIVE_ASSET_ADDRESS)
    elif asset1 == REDO_FRIENDLY_MASTER_ADDRESS:
        layout = (1, asset0 == STON_NATIVE_ASSET_ADDRESS)
    else:
        raise RuntimeError("The configured alert pool does not contain the REDO master contract")
...
def deliver_alert_outbox(runtime: RuntimeConfig, state: dict[str, Any]) -> int:
    ...
    while state.get("ston_alert_outbox"):
        ...
        enabled_key = "buy_alerts_enabled" if side == "BUY" else "sell_alerts_enabled"
        if not bool(state.get(enabled_key, True)):
            state["ston_alert_outbox"] = list(state.get("ston_alert_outbox") or [])[1:]
            save_state(state)
            log(f"Discarded paused {side.lower()} alert {event.event_id} from the outbox")
            continue
        event = enrich_alert_wallet_dns(runtime, event)
        if side == "BUY":
            target_chats = get_buy_alert_channels(state, runtime)
            ...
            target_chats = get_sell_alert_channels(state, runtime)
        ...
        remaining = normalize_alert_channels(item.get("remaining_channels"))
        if not bool(item.get("destinations_initialized")):
            remaining = list(target_chats)
            item["remaining_channels"] = remaining
            item["destinations_initialized"] = True
            save_state(state)
        for target_chat in list(remaining):
        ...
        log(
            ...
            f"to {len(target_chats)} destination(s) for {shorten_wallet(event.wallet_address)} "
            f"from {DEFAULT_ALERT_DEX} durable cursor"
        )
    return delivered
...
def dedust_asset_is_token(asset: Any) -> bool:
    return (
        ...
        and str(asset.get("address") or "").strip() == REDO_FRIENDLY_MASTER_ADDRESS
    )
...
def parse_buy_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[BuyAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_out != REDO_MASTER_ADDRESS or symbol_out != "REDO":
                continue
...
def parse_sell_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[SellAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_in != REDO_MASTER_ADDRESS or symbol_in != "REDO":
                continue
...
def alert_template_values(
    ...
    *,
    ...
    values = {
        ...
        "[REDO_AMOUNT]": format_token_amount(event.utya_amount, 2),
...
def normalize_alert_channels(raw: object) -> list[str]:
...
def parse_alert_channels_input(raw: str) -> list[str]:
    channels = normalize_alert_channels(raw)
    if not channels:
        raise ValueError("Enter at least one channel.")
    return channels
...
def alert_template_prompt_text(state: dict[str, Any], *, side: str) -> str:
    ...
    return (
        ...
        "<code>[PRICE]</code> current REDO price\n"
        ...
        "<code>[REDO_AMOUNT]</code> REDO amount\n"
...
def build_menu_text(state: dict[str, Any], runtime: RuntimeConfig, user_id: str) -> str:
    ...
    lines = [
        ...
        f"• <b>Destinations:</b> <code>{get_buy_alert_channel(state, runtime)}</code>",
        ...
        f"• <b>Destinations:</b> <code>{get_sell_alert_channel(state, runtime)}</code>",
        ...
        return {
            "inline_keyboard": [
                ...
                [
                    ...
                    {"text": "📣 Destinations", "callback_data": "prompt:set_buy_alert_channel"},
                ],
                [
        ...
        return {
            "inline_keyboard": [
                ...
                [
                    ...
                    {"text": "📣 Destinations", "callback_data": "prompt:set_sell_alert_channel"},
                ],
                [
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_buy_alert_utya', True) else '🚫'} REDO", "callback_data": "action:toggle_buy_alert_field:utya"},
                ],
                [
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_sell_alert_utya', True) else '🚫'} REDO", "callback_data": "action:toggle_sell_alert_field:utya"},
                ],
                [
...
def build_menu_payload(state: dict[str, Any], runtime: RuntimeConfig, user_id: str, page: Optional[str] = None) -> tuple[str, dict[str, Any], str]:
    ...
    if active_page == "logs":
        ...
        text = (
            ...
            "Post a whale alert when a REDO buy on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('buy_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('buy_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
            f"• <b>Destinations:</b> <code>{get_buy_alert_channel(state, runtime)}</code>\n"
            f"• <b>Check every:</b> <code>{format_interval(int(state.get('buy_alert_interval_seconds', DEFAULT_ALERT_INTERVAL_SECONDS) or DEFAULT_ALERT_INTERVAL_SECONDS))}</code>\n"
        ...
        text = (
            ...
            f"• <b>REDO amount:</b> <code>{'ON' if state.get('show_buy_alert_utya', True) else 'OFF'}</code>\n"
        ...
        text = (
            ...
            "Post a whale alert when a REDO sell on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('sell_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('sell_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
            f"• <b>Destinations:</b> <code>{get_sell_alert_channel(state, runtime)}</code>\n"
            f"• <b>Check every:</b> <code>{format_interval(int(state.get('sell_alert_interval_seconds', DEFAULT_ALERT_INTERVAL_SECONDS) or DEFAULT_ALERT_INTERVAL_SECONDS))}</code>\n"
        ...
        text = (
            ...
            f"• <b>REDO amount:</b> <code>{'ON' if state.get('show_sell_alert_utya', True) else 'OFF'}</code>\n"
...
def get_buy_alert_channels(state: dict[str, Any], runtime: RuntimeConfig) -> list[str]:
    channels = normalize_alert_channels(state.get("buy_alert_channels"))
    if channels:
        return channels
    legacy_channel = str(state.get("buy_alert_channel") or "").strip()
    return normalize_alert_channels(legacy_channel or runtime.channel)


def get_buy_alert_channel(state: dict[str, Any], runtime: RuntimeConfig) -> str:
    return ", ".join(get_buy_alert_channels(state, runtime))


def get_sell_alert_channels(state: dict[str, Any], runtime: RuntimeConfig) -> list[str]:
    channels = normalize_alert_channels(state.get("sell_alert_channels"))
    if channels:
        return channels
    legacy_channel = str(state.get("sell_alert_channel") or "").strip()
    return normalize_alert_channels(legacy_channel or runtime.channel)


def get_sell_alert_channel(state: dict[str, Any], runtime: RuntimeConfig) -> str:
    return ", ".join(get_sell_alert_channels(state, runtime))
...
def check_buy_alerts(
    ...
    *,
    ...
    for alert in fresh_alerts:
        ...
        message = build_buy_alert_text(state, alert)
...
def check_sell_alerts(
    ...
    *,
    ...
    for alert in fresh_alerts:
        alert = enrich_alert_wallet_dns(runtime, alert)
        message = build_sell_alert_text(state, alert)
        for target_chat in target_chats:
            send_message(runtime, target_chat, message, parse_mode="HTML")
        state["last_sell_alert_at"] = utc_now().isoformat()
...
def handle_text_prompt(runtime: RuntimeConfig, state: dict[str, Any], message: dict[str, Any]) -> bool:
    ...
    try:
        ...
        if action == "set_buy_alert_channel":
            channels = parse_alert_channels_input(text)
            state["buy_alert_channels"] = channels
            state["buy_alert_channel"] = channels[0]
            target_page = "alerts"
            notify_admin_change(
                runtime,
                state,
                actor_id=user_id,
                actor_label=actor_label,
                action=f"changed buy-alert destinations to {', '.join(channels)}.",
            )
            log(f"Buy alert destinations changed to {', '.join(channels)} via bot")
            return True
        ...
        if action == "set_sell_alert_channel":
            channels = parse_alert_channels_input(text)
            state["sell_alert_channels"] = channels
            state["sell_alert_channel"] = channels[0]
            target_page = "sell_alerts"
            notify_admin_change(
                runtime,
                state,
                actor_id=user_id,
                actor_label=actor_label,
                action=f"changed sell-alert destinations to {', '.join(channels)}.",
            )
            log(f"Sell alert destinations changed to {', '.join(channels)} via bot")
            return True
...
def process_message(runtime: RuntimeConfig, state: dict[str, Any], message: dict[str, Any]) -> None:
    ...
    if command == "/setchannel":
        ...
        save_state(state)
        upsert_menu_message(runtime, state, user_id, page="home")
        send_message(runtime, chat_id, "📣 <b>Reply with the new channel</b>\nExamples: <code>@redopricess</code> or <code>-100...</code>", parse_mode="HTML", reply_markup={"force_reply": True, "selective": True}, thread_id=None)
        return
...
def process_callback(runtime: RuntimeConfig, state: dict[str, Any], callback: dict[str, Any]) -> None:
    ...
    if data.startswith("action:toggle_buy_alert_field:"):
        ...
        field_map = {
            ...
            "utya": ("show_buy_alert_utya", "REDO amount"),
    ...
    if data.startswith("action:toggle_sell_alert_field:"):
        ...
        field_map = {
            ...
            "utya": ("show_sell_alert_utya", "REDO amount"),
    ...
    if data == "prompt:set_buy_alert_channel":
        ...
        session["menu_message_id"] = message_id
        upsert_menu_message(runtime, state, user_id, page="alerts")
        send_message(runtime, chat_id, "📣 <b>Reply with the buy-alert destinations</b>\nSend one or more channels separated by commas or new lines.\nExample: <code>@channel1, @channel2</code>.", parse_mode="HTML", reply_markup={"force_reply": True, "selective": True}, thread_id=None)
        return
    ...
    if data == "prompt:set_sell_alert_channel":
        ...
        session["menu_message_id"] = message_id
        upsert_menu_message(runtime, state, user_id, page="sell_alerts")
        send_message(runtime, chat_id, "📣 <b>Reply with the sell-alert destinations</b>\nSend one or more channels separated by commas or new lines.\nExample: <code>@channel1, @channel2</code>.", parse_mode="HTML", reply_markup={"force_reply": True, "selective": True}, thread_id=None)
        return
...
def run() -> int:
    ...
    log(f"Starting REDO price bot -> {runtime.channel} every {runtime.interval_seconds}s")
