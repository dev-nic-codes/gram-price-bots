...
LOG_PATH = ROOT / "scat-price-bot.log"
...
DEFAULT_ALERT_POOL_ADDRESS = "EQAIJOBF6evx5DgOE2DygfwDzRbzK5sdiWzdp1duWCjtgAuP"
DEFAULT_ALERT_POOL_LABEL = "STON.fi SCAT/GRAM"
DEFAULT_ALERT_DEX = "stonfi"
DEFAULT_ALERT_THRESHOLD_USD = 1000.0
DEFAULT_ALERT_INTERVAL_SECONDS = 20
DEFAULT_ALERT_FETCH_LIMIT = 25
SCAT_MASTER_ADDRESS = "0:d5d10f1eb8f3ddb311df68e4009ad743ae1de3e68fe874c60824ce27ba250eea"
SCAT_FRIENDLY_MASTER_ADDRESS = "EQDV0Q8euPPdsxHfaOQAmtdDrh3j5o_odMYIJM4nuiUO6t88"
TOKEN_DECIMALS = 9
...
DEFAULT_CHANNEL = "@ScaredCatsPrice"
DEFAULT_USER_AGENT = "scat-price-bot/2.0"
...
def fetch_dexscreener_market_snapshot_live(runtime: RuntimeConfig) -> MarketSnapshot:
    ...
    if str(base.get("address") or "").strip() != SCAT_FRIENDLY_MASTER_ADDRESS:
...
def fetch_ston_pair_layout(runtime: RuntimeConfig, pool_address: str) -> tuple[int, bool]:
    ...
    if asset0 == SCAT_FRIENDLY_MASTER_ADDRESS:
        layout = (0, asset1 == STON_NATIVE_ASSET_ADDRESS)
    elif asset1 == SCAT_FRIENDLY_MASTER_ADDRESS:
        layout = (1, asset0 == STON_NATIVE_ASSET_ADDRESS)
    else:
        raise RuntimeError("The configured alert pool does not contain the SCAT master contract")
...
def dedust_asset_is_token(asset: Any) -> bool:
    return (
        ...
        and str(asset.get("address") or "").strip() == SCAT_FRIENDLY_MASTER_ADDRESS
    )
...
def parse_buy_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[BuyAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_out != SCAT_MASTER_ADDRESS or symbol_out != "SCAT":
                continue
...
def parse_sell_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[SellAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_in != SCAT_MASTER_ADDRESS or symbol_in != "SCAT":
                continue
...
def alert_template_values(
    ...
    *,
    ...
    values = {
        ...
        "[SCAT_AMOUNT]": format_token_amount(event.utya_amount, 2),
...
def alert_template_prompt_text(state: dict[str, Any], *, side: str) -> str:
    ...
    return (
        ...
        "<code>[PRICE]</code> current SCAT price\n"
        ...
        "<code>[SCAT_AMOUNT]</code> SCAT amount\n"
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_buy_alert_utya', True) else '🚫'} SCAT", "callback_data": "action:toggle_buy_alert_field:utya"},
                ],
                [
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_sell_alert_utya', True) else '🚫'} SCAT", "callback_data": "action:toggle_sell_alert_field:utya"},
                ],
                [
...
def build_menu_payload(state: dict[str, Any], runtime: RuntimeConfig, user_id: str, page: Optional[str] = None) -> tuple[str, dict[str, Any], str]:
    ...
    if active_page == "logs":
        ...
        text = (
            ...
            "Post a whale alert when a SCAT buy on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('buy_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('buy_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
        ...
        text = (
            ...
            f"• <b>SCAT amount:</b> <code>{'ON' if state.get('show_buy_alert_utya', True) else 'OFF'}</code>\n"
        ...
        text = (
            ...
            "Post a whale alert when a SCAT sell on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('sell_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('sell_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
        ...
        text = (
            ...
            f"• <b>SCAT amount:</b> <code>{'ON' if state.get('show_sell_alert_utya', True) else 'OFF'}</code>\n"
...
def process_message(runtime: RuntimeConfig, state: dict[str, Any], message: dict[str, Any]) -> None:
    ...
    if command == "/setchannel":
        ...
        save_state(state)
        upsert_menu_message(runtime, state, user_id, page="home")
        send_message(runtime, chat_id, "📣 <b>Reply with the new channel</b>\nExamples: <code>@ScaredCatsPrice</code> or <code>-100...</code>", parse_mode="HTML", reply_markup={"force_reply": True, "selective": True}, thread_id=None)
        return
...
def process_callback(runtime: RuntimeConfig, state: dict[str, Any], callback: dict[str, Any]) -> None:
    ...
    if data.startswith("action:toggle_buy_alert_field:"):
        ...
        field_map = {
            ...
            "utya": ("show_buy_alert_utya", "SCAT amount"),
    ...
    if data.startswith("action:toggle_sell_alert_field:"):
        ...
        field_map = {
            ...
            "utya": ("show_sell_alert_utya", "SCAT amount"),
...
def run() -> int:
    ...
    log(f"Starting SCAT price bot -> {runtime.channel} every {runtime.interval_seconds}s")
