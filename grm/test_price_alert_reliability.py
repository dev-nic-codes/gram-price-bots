...
class TokenIdentityTests(unittest.TestCase):
    def test_grm_identity_and_default_destination(self) -> None:
        self.assertEqual(main.TOKEN_TICKER, "GRM")
        self.assertEqual(main.COINGECKO_ID, "gram-2")
        self.assertEqual(
            main.GRM_FRIENDLY_MASTER_ADDRESS,
            "EQC47093oX5Xhb0xuk2lCr2RhS8rj-vul61u4W2UH5ORmG_O",
        )
        self.assertEqual(
            main.GRM_MASTER_ADDRESS,
            "0:b8ef4f77a17e5785bd31ba4da50abd91852f2b8febee97ad6ee16d941f939198",
        )
        self.assertEqual(
            main.DEFAULT_ALERT_POOL_ADDRESS,
            "EQASBZLwa2vfdsgoDF2w96pdccBJJRxDNXXPUL7NMm0WdnMx",
        )
        self.assertEqual(main.DEFAULT_CHANNEL, "@GRM_prices")
...
def dedust_trade(*, lt: int, buy: bool) -> dict:
    native = {"type": "native"}
    token = {"type": "jetton", "address": main.GRM_FRIENDLY_MASTER_ADDRESS}
    if buy:
...
class PriceSourceTests(unittest.TestCase):
    def test_primary_pool_price_and_native_gram_rate_are_validated(self) -> None:
        payload = {
            "pairs": [{
                "pairAddress": POOL,
                "baseToken": {"address": main.GRM_FRIENDLY_MASTER_ADDRESS},
                "priceUsd": "0.02",
                "priceNative": "0.01",
...
class DedustTests(unittest.TestCase):
    ...
    def test_dedust_classifies_raw_buy_and_sell_amounts(self) -> None:
        ...
        self.assertEqual(parsed[0][1].grm_amount, 60000)
        self.assertEqual(parsed[1][1].usd_amount, 1200)
