...
LOG_PATH = ROOT / "grm-price-bot.log"
...
DEFAULT_ALERT_POOL_ADDRESS = "EQASBZLwa2vfdsgoDF2w96pdccBJJRxDNXXPUL7NMm0WdnMx"
DEFAULT_ALERT_POOL_LABEL = "STON.fi GRM/GRAM"
DEFAULT_ALERT_DEX = "stonfi"
DEFAULT_ALERT_THRESHOLD_USD = 1000.0
DEFAULT_ALERT_INTERVAL_SECONDS = 20
DEFAULT_ALERT_FETCH_LIMIT = 25
GRM_MASTER_ADDRESS = "0:b8ef4f77a17e5785bd31ba4da50abd91852f2b8febee97ad6ee16d941f939198"
GRM_FRIENDLY_MASTER_ADDRESS = "EQC47093oX5Xhb0xuk2lCr2RhS8rj-vul61u4W2UH5ORmG_O"
TOKEN_DECIMALS = 9
...
DEFAULT_CHANNEL = "@GRM_prices"
DEFAULT_USER_AGENT = "grm-price-bot/2.0"
...
def load_state(env: EnvConfig) -> dict[str, Any]:
    ...
    state["show_buy_alert_grm"] = bool(state.get("show_buy_alert_grm", True))
    ...
    state["show_sell_alert_grm"] = bool(state.get("show_sell_alert_grm", True))
...
def load_alert_market_snapshot() -> MarketSnapshot:
    ...
    grm_price = shared_prices.get(TOKEN_TICKER) or (cached.price_usd if cached else None)
    gram_price = shared_prices.get("GRAM") or (cached.gram_price_usd if cached else None)
    if grm_price is None or grm_price <= 0:
...
def fetch_dexscreener_market_snapshot_live(runtime: RuntimeConfig) -> MarketSnapshot:
    ...
    if str(base.get("address") or "").strip() != GRM_FRIENDLY_MASTER_ADDRESS:
...
def fetch_grm_market_snapshot_live(runtime: RuntimeConfig) -> MarketSnapshot:
    if not COINGECKO_ID:
...
def fetch_grm_market_snapshot(runtime: RuntimeConfig) -> MarketSnapshot:
    errors: list[str] = []
    sources = [("DexScreener primary pool", fetch_dexscreener_market_snapshot_live)]
    if COINGECKO_ID:
        sources.append(("CoinGecko", fetch_grm_market_snapshot_live))
...
def fetch_ston_pair_layout(runtime: RuntimeConfig, pool_address: str) -> tuple[int, bool]:
    ...
    if asset0 == GRM_FRIENDLY_MASTER_ADDRESS:
        layout = (0, asset1 == STON_NATIVE_ASSET_ADDRESS)
    elif asset1 == GRM_FRIENDLY_MASTER_ADDRESS:
        layout = (1, asset0 == STON_NATIVE_ASSET_ADDRESS)
    else:
        raise RuntimeError("The configured alert pool does not contain the GRM master contract")
    ...
    for item in ordered_events:
        ...
        for side, (enabled, pool_address, pool_label, threshold_usd) in configured.items():
            if not enabled or pair_id != pool_address:
                continue
            grm_index, other_is_gram = pool_layouts[pool_address]
            other_index = 1 - grm_index
            if side == "BUY":
                grm_amount = parse_positive_amount(item.get(f"amount{grm_index}Out"))
                counter_amount = parse_positive_amount(item.get(f"amount{other_index}In"))
            else:
                grm_amount = parse_positive_amount(item.get(f"amount{grm_index}In"))
                counter_amount = parse_positive_amount(item.get(f"amount{other_index}Out"))
            if grm_amount is None or counter_amount is None:
                continue
            if other_is_gram:
                gram_amount = counter_amount
                usd_amount = gram_amount * gram_price
            else:
                usd_amount = grm_amount * snapshot.price_usd
...
def dedust_asset_is_token(asset: Any) -> bool:
    return (
        ...
        and str(asset.get("address") or "").strip() == GRM_FRIENDLY_MASTER_ADDRESS
    )
...
def parse_dedust_alert_events(
    ...
    for item in sorted(trades, key=lambda trade: int(trade.get("lt") or 0)):
        ...
        if dedust_asset_is_native(asset_in) and dedust_asset_is_token(asset_out):
            ...
            grm_amount = parse_dedust_raw_amount(item.get("amountOut"), TOKEN_DECIMALS)
        elif dedust_asset_is_token(asset_in) and dedust_asset_is_native(asset_out):
            side = "SELL"
            grm_amount = parse_dedust_raw_amount(item.get("amountIn"), TOKEN_DECIMALS)
            gram_amount = parse_dedust_raw_amount(item.get("amountOut"), 9)
        else:
            continue
        enabled, configured_pool, pool_label, threshold_usd = configured[side]
        if not enabled or configured_pool != pool_address or gram_amount is None or grm_amount is None:
            continue
        usd_amount = gram_amount * gram_price
        if usd_amount < threshold_usd:
            continue
...
def build_buy_alert_dedupe_key(event: BuyAlertEvent) -> str:
    ...
    amount_cents = int(round(float(event.grm_amount) * 100))
...
def parse_buy_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[BuyAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_out != GRM_MASTER_ADDRESS or symbol_out != "GRM":
                continue
            amount_out_raw = str(swap.get("amount_out") or "").strip()
            decimals = int(jetton_out.get("decimals", 9) or 9)
            try:
                grm_amount = int(amount_out_raw) / (10 ** max(0, decimals))
            except Exception:
                continue
            fallback_usd_amount = float(grm_amount * snapshot.price_usd)
...
def parse_sell_alert_events(state: dict[str, Any], events: list[dict[str, Any]], snapshot: MarketSnapshot) -> list[SellAlertEvent]:
    ...
    for event in reversed(events):
        ...
        for action in list(event.get("actions") or []):
            ...
            if master_in != GRM_MASTER_ADDRESS or symbol_in != "GRM":
                continue
            amount_in_raw = str(swap.get("amount_in") or "").strip()
            decimals = int(jetton_in.get("decimals", 9) or 9)
            try:
                grm_amount = int(amount_in_raw) / (10 ** max(0, decimals))
            except Exception:
                continue
            fallback_usd_amount = float(grm_amount * snapshot.price_usd)
            gram_amount, usd_amount = gram_values_for_swap(
...
def alert_template_values(
    ...
    *,
    ...
    values = {
        "[SIDE]": side.upper(),
        "[PRICE]": format_alert_price(event.usd_amount, event.grm_amount),
        ...
        "[GRM_AMOUNT]": format_token_amount(event.grm_amount, 2),
        "[TOKEN_AMOUNT]": format_token_amount(event.grm_amount, 2),
        "[WALLET]": wallet,
        "[SHORT_WALLET]": short_wallet,
        "[SHORT_ADDRESS]": short_address,
        "[WALLET_SHORT]": short_wallet,
...
def alert_template_prompt_text(state: dict[str, Any], *, side: str) -> str:
    ...
    return (
        ...
        "<code>[PRICE]</code> current GRM price\n"
        ...
        "<code>[GRM_AMOUNT]</code> GRM amount\n"
...
def build_menu_text(state: dict[str, Any], runtime: RuntimeConfig, user_id: str) -> str:
    ...
    alert_display_flags = " | ".join(
        [
            ...
            f"grm:{'on' if state.get('show_buy_alert_grm', True) else 'off'}",
    ...
    sell_alert_display_flags = " | ".join(
        [
            ...
            f"grm:{'on' if state.get('show_sell_alert_grm', True) else 'off'}",
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_buy_alert_grm', True) else '🚫'} GRM", "callback_data": "action:toggle_buy_alert_field:grm"},
                ],
                [
        ...
        return {
            "inline_keyboard": [
                [
                    ...
                    {"text": f"{'✅' if state.get('show_sell_alert_grm', True) else '🚫'} GRM", "callback_data": "action:toggle_sell_alert_field:grm"},
                ],
                [
...
def build_menu_payload(state: dict[str, Any], runtime: RuntimeConfig, user_id: str, page: Optional[str] = None) -> tuple[str, dict[str, Any], str]:
    ...
    if active_page == "logs":
        ...
        text = (
            ...
            "Post a whale alert when a GRM buy on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('buy_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('buy_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
        ...
        text = (
            ...
            f"• <b>GRM amount:</b> <code>{'ON' if state.get('show_buy_alert_grm', True) else 'OFF'}</code>\n"
        ...
        text = (
            ...
            "Post a whale alert when a GRM sell on the main pool crosses your USD threshold.\n\n"
            f"• <b>Status:</b> <code>{'ON' if state.get('sell_alerts_enabled', True) else 'OFF'}</code>\n"
            f"• <b>Threshold:</b> <code>{format_usd_value(float(state.get('sell_alert_threshold_usd', DEFAULT_ALERT_THRESHOLD_USD) or DEFAULT_ALERT_THRESHOLD_USD))}</code>\n"
        ...
        text = (
            ...
            f"• <b>GRM amount:</b> <code>{'ON' if state.get('show_sell_alert_grm', True) else 'OFF'}</code>\n"
...
def post_price(runtime: RuntimeConfig, state: dict[str, Any], *, reason: str) -> str:
    snapshot = fetch_grm_market_snapshot(runtime)
...
def check_buy_alerts(
    ...
    *,
    ...
    snapshot = snapshot or fetch_grm_market_snapshot(runtime)
...
def process_message(runtime: RuntimeConfig, state: dict[str, Any], message: dict[str, Any]) -> None:
    ...
    if command == "/setchannel":
        ...
        save_state(state)
        upsert_menu_message(runtime, state, user_id, page="home")
        send_message(runtime, chat_id, "📣 <b>Reply with the new channel</b>\nExamples: <code>@GRM_prices</code> or <code>-100...</code>", parse_mode="HTML", reply_markup={"force_reply": True, "selective": True}, thread_id=None)
        return
...
def process_callback(runtime: RuntimeConfig, state: dict[str, Any], callback: dict[str, Any]) -> None:
    ...
    if data.startswith("action:toggle_buy_alert_field:"):
        ...
        field_map = {
            ...
            "grm": ("show_buy_alert_grm", "GRM amount"),
    ...
    if data.startswith("action:toggle_sell_alert_field:"):
        ...
        field_map = {
            ...
            "grm": ("show_sell_alert_grm", "GRM amount"),
...
def run() -> int:
    ...
    log(f"Starting GRM price bot -> {runtime.channel} every {runtime.interval_seconds}s")
