...
class TokenIdentityTests(unittest.TestCase):
    def test_gramming_identity_and_default_destination(self) -> None:
        self.assertEqual(main.TOKEN_TICKER, "GRAMMING")
        self.assertEqual(main.COINGECKO_ID, "gramming")
        self.assertEqual(
            main.GRAMMING_FRIENDLY_MASTER_ADDRESS,
            "EQAmsYIAadPQrEn-wZrRKwqhnReLDOeKl9T70umuk0MA1ULW",
        )
        self.assertEqual(
            main.GRAMMING_MASTER_ADDRESS,
            "0:26b1820069d3d0ac49fec19ad12b0aa19d178b0ce78a97d4fbd2e9ae934300d5",
        )
        self.assertEqual(
            main.DEFAULT_ALERT_POOL_ADDRESS,
            "EQAxIrZcBnEtaIWvgWdq7lDY__uNDG-SSGSNQRuAm7XWv4HV",
        )
        self.assertEqual(main.DEFAULT_ALERT_DEX, "dedust")
        self.assertEqual(main.DEFAULT_CHANNEL, "@grammingprices")
...
def dedust_trade(*, lt: int, buy: bool) -> dict:
    native = {"type": "native"}
    token = {"type": "jetton", "address": main.GRAMMING_FRIENDLY_MASTER_ADDRESS}
    if buy:
...
class PriceSourceTests(unittest.TestCase):
    def test_primary_pool_price_and_native_gram_rate_are_validated(self) -> None:
        payload = {
            "pairs": [{
                "pairAddress": POOL,
                "baseToken": {"address": main.GRAMMING_FRIENDLY_MASTER_ADDRESS},
                "priceUsd": "0.02",
                "priceNative": "0.01",
...
class DedustTests(unittest.TestCase):
    ...
    def test_dedust_classifies_raw_buy_and_sell_amounts(self) -> None:
        ...
        self.assertEqual(parsed[0][1].gramming_amount, 60000)
        self.assertEqual(parsed[1][1].usd_amount, 1200)
    ...
    def test_empty_dedust_poll_updates_health_timestamp(self) -> None:
        state = alert_state()
        state["dedust_alert_last_error"] = "old timeout"
        with tempfile.TemporaryDirectory() as directory, patch.object(
            main, "STATE_PATH", Path(directory) / "state.json"
        ), patch.object(
            main, "load_alert_market_snapshot", return_value=self.snapshot
        ), patch.object(main, "fetch_dedust_trades", return_value=[]):
            queued, delivered = main.sync_dedust_alerts(self.runtime, state)
        self.assertEqual((queued, delivered), (0, 0))
        self.assertTrue(state["dedust_alert_last_sync_at"])
        self.assertEqual(state["dedust_alert_last_error"], "")
