...
def dedust_trade(*, lt: int, buy: bool) -> dict:
    native = {"type": "native"}
    token = {"type": "jetton", "address": main.UTYA_FRIENDLY_MASTER_ADDRESS}
    if buy:
...
class PriceSourceTests(unittest.TestCase):
    def test_primary_pool_price_and_native_gram_rate_are_validated(self) -> None:
        payload = {
            "pairs": [{
                "pairAddress": POOL,
                "baseToken": {"address": main.UTYA_FRIENDLY_MASTER_ADDRESS},
                "priceUsd": "0.02",
                "priceNative": "0.01",
...
class DeliveryTests(unittest.TestCase):
    ...
    def test_buy_alert_is_sent_to_every_configured_destination(self) -> None:
        state = alert_state()
        state["buy_alert_channels"] = ["@one", "@two"]
        event = main.BuyAlertEvent(
            "event", "wallet", 60000, 1500, "pool", POOL, "tx", 123, "-", 1000
        )
        state["ston_alert_outbox"] = [main.serialize_alert_outbox_item("BUY", event)]
        ...
        with tempfile.TemporaryDirectory() as directory, patch.object(
            ...
            delivered = main.deliver_alert_outbox(runtime, state)
        self.assertEqual(delivered, 1)
        self.assertEqual([call.args[1] for call in send_message.call_args_list], ["@one", "@two"])
        self.assertEqual(state["ston_alert_outbox"], [])

    def test_retry_resumes_at_first_destination_that_failed(self) -> None:
        state = alert_state()
        state["buy_alert_channels"] = ["@one", "@two"]
        event = main.BuyAlertEvent(
            "event", "wallet", 60000, 1500, "pool", POOL, "tx", 123, "-", 1000
        ...
        with tempfile.TemporaryDirectory() as directory, patch.object(
            ...
            main, "send_message", side_effect=[{}, RuntimeError("temporary failure")]
        ):
            with self.assertRaisesRegex(RuntimeError, "temporary failure"):
                main.deliver_alert_outbox(runtime, state)
            self.assertEqual(state["ston_alert_outbox"][0]["remaining_channels"], ["@two"])
            with patch.object(main, "send_message") as retry_send:
                delivered = main.deliver_alert_outbox(runtime, state)
        self.assertEqual(delivered, 1)
        self.assertEqual([call.args[1] for call in retry_send.call_args_list], ["@two"])


class DestinationTests(unittest.TestCase):
    def test_multiple_destinations_are_validated_and_deduplicated(self) -> None:
        self.assertEqual(
            main.parse_alert_channels_input("@one, @two\n@one; -100123"),
            ["@one", "@two", "-100123"],
        )
