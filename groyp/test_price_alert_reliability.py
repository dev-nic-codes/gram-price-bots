...
class TokenIdentityTests(unittest.TestCase):
    def test_groyp_identity_and_default_destination(self) -> None:
        self.assertEqual(main.TOKEN_TICKER, "GROYP")
        self.assertEqual(main.COINGECKO_ID, "groyper-3")
        self.assertEqual(
            main.GROYP_FRIENDLY_MASTER_ADDRESS,
            "EQAtwo6qMNwtr0iTA9eKVZ32cuACFJ0VKd78GrBWOe83-X1P",
        )
        self.assertEqual(
            main.GROYP_MASTER_ADDRESS,
            "0:2dc28eaa30dc2daf489303d78a559df672e002149d1529defc1ab05639ef37f9",
        )
        self.assertEqual(
            main.DEFAULT_ALERT_POOL_ADDRESS,
            "EQDPtuvcV-lq_i3aYT9ZCEOKIj4EwcrnsBJjMJmQpehBeSPD",
        )
        self.assertEqual(main.DEFAULT_CHANNEL, "@groypprices")
...
def dedust_trade(*, lt: int, buy: bool) -> dict:
    native = {"type": "native"}
    token = {"type": "jetton", "address": main.GROYP_FRIENDLY_MASTER_ADDRESS}
    if buy:
...
class PriceSourceTests(unittest.TestCase):
    def test_primary_pool_price_and_native_gram_rate_are_validated(self) -> None:
        payload = {
            "pairs": [{
                "pairAddress": POOL,
                "baseToken": {"address": main.GROYP_FRIENDLY_MASTER_ADDRESS},
                "priceUsd": "0.02",
                "priceNative": "0.01",
...
class DedustTests(unittest.TestCase):
    ...
    def test_dedust_classifies_raw_buy_and_sell_amounts(self) -> None:
        ...
        self.assertEqual(parsed[0][1].groyp_amount, 60000)
        self.assertEqual(parsed[1][1].usd_amount, 1200)
